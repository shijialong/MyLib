...
/***** 设置UI *****/
#define KAlertViewWidth                         260 * SCREEN_WIDTH / 320
...
#define KSpaceSmall                             5
#define KSpaceLarge                             20
...
+ (void)showAlertViewWithTitle:(NSString *)title contentView:(UIView *)contentView message:(NSString *)message buttonTitles:(NSArray<__kindof NSString *> *)buttonTitles buttonClickedBlock:(AlertViewButtonClickedBlock)buttonClickedBlock {
    JLAlertView *alert = [[JLAlertView alloc] initWithTitle:title contentView:contentView message:message buttonTitles:buttonTitles buttonClickedBlock:buttonClickedBlock];
    [alert show];
}
...
- (void)setupAlertView {
    ...
    if (_title != nil && _title.length > 0) {
        ...
        alertViewHeight += _titleLabel.frame.size.height + KSpaceLarge;
    }
    /***** 设置自定义contentView *****/
    if (_contentView) {
